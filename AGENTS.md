...
## Project Development Rules（健身助手项目）

**⚠️ 强制约束：本项目所有开发活动必须遵守以下规则**

### 开发前必读（顺序）

每次涉及开发任务前，**必须按顺序读取**：

1. **`docs/product/dev-rules.md`** — 开发准入门槛和工作流程
2. **`docs/product/fitness-assistant-prd.md`** — 产品需求定义  
3. **`docs/product/backlog.md`** — 可执行任务清单
4. **`docs/product/decisions.md`** — 技术决策记录
...
```
检查清单：
□ PRD 是否存在且最新？（docs/product/*-prd.md）
□ Backlog 是否有可执行任务？（docs/product/backlog.md）
□ 相关决策是否已记录？（docs/product/decisions.md）
...
| 优先级 | Rule 类型 | 文件路径 | 内容 |
|-------|---------|---------|------|
| 1 | **开发准入规则** | `docs/product/dev-rules.md` | 开发门槛、工作流程、自检清单 |
| 2 | **PRD 规范** | `rules/product/prd_policy.md` | PRD 最小字段、checklist 规则 |
| 3 | **迭代管理规则** | `rules/dev/iteration_and_pm.md` | 迭代日志格式、backlog 要求、ADR-lite |

**注意：这些是本项目的强制规则文件，开发前必须读取并遵守。**
